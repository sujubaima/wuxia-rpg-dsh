...
function layoutSceneGraph(adj,root){
  ...
  pos.set(root,[0,0]);occ.add(key(0,0));
  const q=[root];parent[root]=null;
  while(q.length){
    const a=q.shift();
...
const SGW=170,SGH=100,SNW=130,SNH=44;
function segCrossBox(x1,y1,x2,y2,bx,by){
  const xmin=bx-SNW/2,xmax=bx+SNW/2,ymin=by-SNH/2,ymax=by+SNH/2;
...
function edgePathD(x1,y1,x2,y2,obstacles){
  const L=Math.hypot(x2-x1,y2-y1);
  if(L===0)return `M ${x1} ${y1} L ${x2} ${y2}`;
  const ux=(x2-x1)/L,uy=(y2-y1)/L,px=-uy,py=ux;
  let reqPos=0,reqNeg=0;
  for(const [ox,oy] of obstacles){
    if(!segCrossBox(x1,y1,x2,y2,ox,oy))continue;
    const t=Math.min(.9,Math.max(.1,((ox-x1)*ux+(oy-y1)*uy)/L));
    const h=(ox-x1)*px+(oy-y1)*py;
    const support=(SNW/2)*Math.abs(px)+(SNH/2)*Math.abs(py);
    const f=Math.max(.5,4*t*(1-t));
  ...
  const clean=(dev,s)=>{
    ...
    for(let i=0;i<=n;i++){
      ...
      for(const [ox,oy] of obstacles)
        if(Math.abs(bx-ox)<SNW/2+2&&Math.abs(by-oy)<SNH/2+2)return false;
    }
    return true;
  };
  ...
  const mx=(x1+x2)/2+px*bestS*bestDev*2,my=(y1+y2)/2+py*bestS*bestDev*2;
...
function renderSceneSvg(adj,cur,stationExit){
  ...
  const W=(maxX-minX+1)*SGW,H=(maxY-minY+1)*SGH;
  const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
  svg.setAttribute('viewBox',`0 0 ${W} ${H}`);
  svg.setAttribute('width',W);svg.setAttribute('height',H);
  svg.style.display='block';
  const cx=x=>(x-minX)*SGW+SGW/2, cy=y=>(y-minY)*SGH+SGH/2;
  ...
  for(const [a,b] of edges){
    ...
    ln.setAttribute('d',edgePathD(ax,ay,bx,by,obstacles));
