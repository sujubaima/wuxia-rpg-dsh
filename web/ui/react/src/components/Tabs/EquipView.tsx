...
export function EquipView() {
  ...
  const refresh = useCallback(async () => {
    setD(await fetchUi({ 类型: '配置装备', ...(role ? { 角色: role } : {}) }))
  }, [role])
  useEffect(() => { void refresh() }, [refresh])
  const withRole = (a: Action) => role ? { ...a, 角色: role } : a
  const err = d?.错误
          ...
          <div className="slotrow">
            {EQUIP_SLOTS.map(s => (
              <div className="slotcard" key={s}>
                ...
                {cur[s] && <Btn onClick={() => uiOp(withRole({ 类型: '配置装备', 操作: '脱', 槽位: s }), { refresh })}>卸下</Btn>}
              </div>
            ))}
          </div>
          <h4>物品栏可换装备</h4>
          {avail.length === 0 ? <div className="ph">（无可换装备）</div> : (
            <UiTable
              ...
              rows={avail.map(it => {
                ...
                const ops = isWeapon ? (
                  <span key="w">
                    <Btn onClick={() => uiOp(withRole({ 类型: '配置装备', 操作: '穿', 槽位: '武器1', 物品: it.名称 }), { refresh })}>装到武器1</Btn>{' '}
                    <Btn onClick={() => uiOp(withRole({ 类型: '配置装备', 操作: '穿', 槽位: '武器2', 物品: it.名称 }), { refresh })}>装到武器2</Btn>
                  </span>
                ) : (() => {
                  const sl = it.类型 && EQUIP_SLOTS.includes(it.类型) ? it.类型 : slotOfSub(it.子类型)
                  return <Btn key="p" disabled={!sl} onClick={() => sl && uiOp(withRole({ 类型: '配置装备', 操作: '穿', 槽位: sl, 物品: it.名称 }), { refresh })}>穿上</Btn>
                })()
                return [
