"""存档与状态读写包:存档持久化 + go/judge/check/prepare 交换文件 IO。
...
进程间交换文件 IO(go/judge/check/prepare 跨进程传递,按生命周期清理,不随存档走):
...
- scene_drafts slot/.runtime/scene_drafts.json(本轮最新场景规划批次,judge 成功或读档后清理)
