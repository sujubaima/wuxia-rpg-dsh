...
```text
玩家输入 → 剧情可执行性预检 → go → 有界面则渲染；无界面则判定/推演 → 必要时 quest-prepare → judge → 渲染 → 等待输入
```
...
- 返回 `状态冲突: go_already_committed` 时沿用 `go_result` 继续 judge；quest-prepare 或 judge 报错时只修正对应请求，绝不重跑 go。judge 失败后草稿仍可重用；judge 成功或读档后草稿失效。
