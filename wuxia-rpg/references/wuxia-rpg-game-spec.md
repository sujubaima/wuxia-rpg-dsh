...
- **先结算后叙事**：玩家指令先完成剧情可执行性预检再 go；按因果完成必要的 `wuxia_check` / `wuxia_random_event`；地图有变化先调 `wuxia_scene_prepare`，即兴线索有变化再调 `wuxia_quest_prepare`；基于真实结果推演后 judge，最后渲染。
...
4. **场景**：本轮是否新增地点、解锁孤点、隔离或重连路径？若是，是否已批量 `scene-prepare`，并在 judge 用 `场景-采用草稿` 整批采用？`场景要素`是否为非空数组并描述落定后的新场景（缺省或空数组会被打回）？最终叙事中新提及且可前往的地点是否都写入 `提及地点`？
5. **线索**：本轮判定与随机结果是否已明确？反向自检：本轮玩家获知的见闻中是否含未解事件的可追查情报？有则当轮建栏，不得以“传闻未成形”为由推迟。新建/扩展线索是否一次批量 `quest-prepare`，并在 judge 用 `线索-采用草稿.名称列表` 采用？新蓝图是否留了扩展点（建议）？已确认的调查、说服、真伪等结果是否提交对应 `事实`，而非直接改阶段或重传线索栏？
