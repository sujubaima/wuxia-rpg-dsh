...
- 三层战报逐回合分段；第一层按结果润色，第三层数值完整，禁止原样粘贴引擎草稿或省略数值。
- `玩家界面`原样呈现，不改写可用武学、物品或行动提示，不提供战术建议。
- `战报`只使用首次返回的完整数据；禁止为补全文本重调战斗 action。
- 战局状态非进行中时必须完成终局界面和战后处置，不得直接跳回 exploration-ui。

战报格式与非我方胜终局见 [battle-ui.md](./ui/wuxia-rpg-battle-ui.md)；我方胜利的处决界面与战后时序见 [battle-end-ui.md](./ui/wuxia-rpg-battle-end-ui.md)。
