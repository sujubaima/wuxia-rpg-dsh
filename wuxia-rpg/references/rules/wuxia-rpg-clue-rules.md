...
`quest-prepare` 不创建正式线索、不改事实或奖励；失败可修正后重试，只有最终 `judge` 成功才生效。
...
进入扩展点前，GM 必须在 `quest-prepare.任务` 中加入“扩展”或“关闭”项，再在同轮 `judge` 以线索名称采用。
...
- `任务扩展`：线索已抵达扩展前沿，须先用 `quest-prepare` 准备扩展，再由 `judge` 采用；该方向无继续推演必要时可改用「关闭」操作强关扩展点。
