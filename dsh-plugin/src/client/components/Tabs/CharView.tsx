...
export function CharView() {
  ...
  if (!resolved) {
    return (
      <CardShell title="角色" desc="探索态尚未就绪——请先推进至少一回合游历，或重调一次。">
  ...
  if (!info) {
    ...
    return <CardShell title="角色" desc="属性为武学反哺+装备加成后的派生值。"><DumpJson data={d} /></CardShell>
  }
  ...
  return (
    <CardShell title="角色" desc="属性为武学反哺+装备加成后的派生值。">
      {kvGrid(base)}
