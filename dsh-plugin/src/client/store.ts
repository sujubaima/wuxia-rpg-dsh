...
import type {
  ...
  SceneElement, Seg, SpecialCommand, TabKey, Toast, UiModalKind, UiModalState, WaitLine,
} from './types'
...
export interface StageState {
  ...
  changes: string[]
  // 展开后的内容
  elements: { 主体: string; 描写: string; 特殊指令: SpecialCommand[] | null }[]
...
interface GameState {
  ...
  uiModalGo: (action: Action, opts?: { confirm?: string; narrate?: string }) => Promise<void>
...
function normEl(f: SceneElement): { 主体: string; 描写: string; 特殊指令: SpecialCommand[] | null } {
  if (f && typeof f === 'object') {
    const raw = f.特殊指令
    const cmds = Array.isArray(raw)
      ? (raw as any[])
          .filter(c => c && typeof c === 'object' && typeof c.名称 === 'string' && c.名称.trim() && typeof c.可用 === 'boolean')
          .map(c => ({ 名称: String(c.名称).trim(), 可用: Boolean(c.可用) }) as SpecialCommand)
      : null
