...
// dsh 版：engine service 跨域（dsh web 在 :3080，engine 在 :8000），用可配 base URL。
import type { Action, EngineResult } from './types'

/** engine service 基址。dsh 下跨域，由插件 config 注入；默认同源（独立 webui 用）。 */
let ENGINE_BASE = ''
export function setEngineBase(url: string): void {
  ENGINE_BASE = url ? url.replace(/\/$/, '') : ''
}

/** go 直通统一 operation 端点。 */
export async function engineGo(actions: Action[], slotOverride?: number): Promise<EngineResult> {
  return enginePost('go', { 槽位: slotOverride != null ? slotOverride : currentSlotRef(), 行为: actions })
}

/** judge 直通统一 operation 端点。 */
export async function engineJudge(entries: Action[], extra?: Record<string, unknown>): Promise<EngineResult> {
  return enginePost('judge', Object.assign({ 槽位: currentSlotRef(), 行为: entries }, extra || {}))
}
...
async function enginePost(operation: 'go' | 'judge', body: Record<string, unknown>): Promise<EngineResult> {
  const resp = await fetch(`${ENGINE_BASE}/api/v1/operations/${operation}`, {
    method: 'POST',
...
export function streamChat(message: string, sessionId: string, h: StreamHandlers): AbortController {
  ...
  void (async () => {
    let resp: Response
    try {
      resp = await fetch(`${ENGINE_BASE}/api/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
